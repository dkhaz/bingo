...
PROGRAM EVALUATION
  ...
  INTEGER::IFAIL,kpoints,neval,last,i,j,modeelements,jend,jstart,ISTART,IEND,INTERVAL
  ...
  allocate(arrayktwobykone(KPOINTS))
  do i=1,KPOINTS
    arrayktwobykone(i)=0.5d0+(1.0d0-0.5d0)*dble(i-1)/dble(KPOINTS-1)
  end do
  ...
  DO I=ISTART,IEND,INTERVAL
     ...
     if(scalene.and.compute_triangle) then 
     
      twobythree=arrayktwobykone(I)
      
      modek2=twobythree*modek3
     ...
     JEND=KPOINTS
#ifdef MPI
     JEND=nint(400*(twobythree-0.5d0))+1
#endif     
     !if(scalene) JEND=I
     DO J=1,JEND
     ...
     if(scalene.and.compute_triangle) then 
      ...
      onebythree=(1.0d0-twobythree)+dble(J)*(2.0d0*twobythree-1.0d0)/dble(JEND)
            
	
      modek1=onebythree*modek3
...
#ifndef MPI
          if(scalene) WRITE(903,*)modek1/modek3,modek2/modek3,DBLE(F_NL)
#endif
#ifdef MPI
          if(scalene) WRITE(903+MY_ID,*)modek1/modek3,modek2/modek3,DBLE(F_NL)
#endif
